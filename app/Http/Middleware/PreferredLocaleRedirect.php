...
class PreferredLocaleRedirect extends LaravelLocalizationMiddlewareBase
{
    /**
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (isset(Auth::user()->profile->preferredLanguage)){
            $prefLocale = Auth::user()->profile->preferredLanguage->locale;
            if (
                $prefLocale &&
                app('laravellocalization')->checkLocaleInSupportedLocales($prefLocale) &&
                !(app('laravellocalization')->isHiddenDefault($prefLocale))
            ){
                $locale = $prefLocale;
            }
        }
