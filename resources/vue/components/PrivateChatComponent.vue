<template>
    <div class="wrapper priv-cht-wrapper">
        <div class="container priv-cht-container">
            <div class="left">
                <div class="top">
                    ...
                    <img style="height: 46px;     float: left;" :src="auth.image" alt="" />
                        ...
                        <span class="dropdown search" style="float: left; margin-right: 10px;top: 13px;left: 20px" aria-hidden="true">
                          ...
                          <div v-if="auth.username<11">{{ auth.username }}</div>
                           <div v-else>  {{auth.username.substring(0,11)+".."}}</div>
            ...
            <div class="chat-wrapper" v-for="friend in filteredFriends" :key="friend.id">
        ...
        <div class="container priv-admin-container">
            <div class="priv-admin">
                <admin-message-component :current="auth"></admin-message-component>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        computed : {
            filteredFriends: function () {
                return this.friends.filter(friend => friend.session && friend.session.open);
            }
        },
        methods: {
            close(friend) {
            ...
            listenForEverySession(friend) {
                console.log(friend);
                this.echo.private(`Chat.${friend.session.id}`).listen(
                    "PrivateChatEvent",
        ...
        created() {
            ...
            this.echo.channel("Chat").listen("SessionEvent", (e) => {
        ...
        components: { PrivateMessageComponent, AdminMessageComponent }
    };
</script>
